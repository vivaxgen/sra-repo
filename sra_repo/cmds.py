...
def input_args(p):
    ...
    p.add_argument("--idfile", default=None, help="file containing list of SRA IDs")
    p.add_argument(
        "--samplefile",
        default=None,
        help="a tab-delimited sample manifest and their " "associated SRA IDs",
    )
    p.add_argument("SRAIDs", nargs="*", help="list of SRA IDs")


def init_argparse():
    p = argparse.ArgumentParser(description="SRA-repo")

    cmds = p.add_subparsers(required=True, dest="command")

    # command: store
    cmd_store = cmds.add_parser("store", help="store read file(s) to database")
    cmd_store.add_argument("infiles", nargs="+")

    # command: path
    cmd_path = cmds.add_parser("path", help="show file path")
    cmd_path.add_argument(
        "--newline",
        default=False,
        action="store_true",
    ...
    # command: path
    cmd_info = cmds.add_parser("info", help="show SRA information")
    input_args(cmd_info)

    # command: check
    cmd_check = cmds.add_parser("check", help="check SRA ACCID in database")
    ...
    cmd_check.add_argument(
        "--count", default=-1, type=int, help="number of SRA IDs to be checked"
    )
    site_args(cmd_check)
    input_args(cmd_check)

    # command: link
    cmd_link = cmds.add_parser(
        "link", help="link read files(s) from sample file " "or SRA id file"
    )
    ...
    cmd_link.add_argument(
        "--non-flat", default=False, action="store_true", help="use directory per SRA"
    )
    cmd_link.add_argument("--outdir", required=True, help="output directory")
    input_args(cmd_link)

    # command: fetch
    cmd_fetch = cmds.add_parser("fetch", help="fetch  read files by ACC IDs")
    ...
    # command: list
    cmd_list = cmds.add_parser("list", help="list SRA IDs in DB")
    ...
    cmd_list.add_argument("patterns", nargs="*", help="pattern for SRA ids")
    cmd_list.add_argument("-o", "--outfile", default=None, help="output file name")

    # command: inventory
    cmd_inventory = cmds.add_parser("inventory", help="view inventory information")
    cmd_inventory.add_argument(
        "--species",
        default=False,
        action="store_true",
    ...
    cmd_convert_accid = cmds.add_parser(
        "convert-accid", help="convert any term to SRA run ID"
    )
    cmd_convert_accid.add_argument("--samplefile", help="sample file with ENA IDs")
    cmd_convert_accid.add_argument("--outfile", help="output file")
...
def do_check(args, fs):
    ...
    validator = sra_validator.SRA_Validator(
        sraids, fs, helpers=helpers, validate=args.validate, showcmds=args.showcmds
    )
    ...
    if args.validate:
        cerr(
            f"{validator.finished - len(errors)} SRA ID(s) have been validated successfully."
        )
    else:
        cerr(
            f"{validator.finished - len(errors)} SRA ID(s) are in repository "
...
def do_link(args, fs):
    ...
    if args.samplefile:
        ...
        for sample, sra_ids in iter_samplefile(args.samplefile):
            ...
            for sra_id in sra_ids:
                ...
                try:
                    ...
                    cerr(
                        f"WARN: SRA ID {sra_id} is not in the database. "
                        f"Please run ena-repo fetch first."
                    )

                except FileExistsError as err:
                    cerr(
                        f"WARN: file {err.filename2} for SRA {sra_id} is already exist "
        ...
        cerr(
            f"INFO: linked {len(fastq_files)} paired FASTQ files for {len(samples)} sample(s) "
        )
        ...
        for sra_id in sraids:
            try:
                read_files += fs.link(
                    sra_id, outdir, dryrun=args.check, flat=(not args.non_flat)
                )
                counter += 1
                ...
                cerr(
                    f"ERR: file {err.filename2} for SRA {sra_id} is already exist "
                    f"in directory: {outdir}"
                )
...
def do_list(args, fs):
    ...
    if args.outfile:
        with open(args.outfile, "w") as f:
            for sra_path in sra_list:
                f.write(f"{sra_path.name}\n")
        cerr(f"List of SRAs written to {args.outfile}")
...
def do_info(args, fs):
    ...
    for sra_id in SRAIDs:
        ...
        try:
            ...
            err_list.append(
                f"{sra_id} does not have info file. "
...
def do_convert_accid(args, fs):

    from sra_repo import entrez_helper

    sample_iter = SampleFileIterator(args.samplefile)

    d = {}
    for sample, accs in sample_iter:
        sra_ids = []
        for acc in accs:
            # get the SRA database ID key
            cerr(f"Converting {acc} to SRA run accession ID")
            root = entrez_helper.get_xml_entry(
                None, payload=dict(db="sra", term=acc), url=entrez_helper.ESEARCH_URL
            )
            id_elements = root.findall(".//IdList/Id")
            id_list = [x.text for x in id_elements]

            # convert SRA database ID to SRA run accession ID

            for db_id in id_list:
                cerr(f"Fetching id: {db_id} to get SRA run accession ID")
                root = entrez_helper.get_xml_entry(db_id)
                sra_ids += [r.attrib["accession"] for r in root.findall("*//RUN")]

        d[sample] = sra_ids

    df = sample_iter.df
    for sample, sra_ids in d.items():
        df.loc[df[sample_iter.sample_column] == sample, sample_iter.ena_column] = (
            ",".join(sra_ids)
        )

    if args.outfile:
        df.to_csv(args.outfile, index=False, sep="\t")
        cerr(f"INFO: written to {args.outfile}")
...
def do_fetch(args, fs):
    ...
    if any(fetcher.sra_errors) or any(fetcher.sra_d):
        ...
        cerr(
            f"WARNING: there are unsuccessful {len(fetcher.sra_errors) + len(fetcher.sra_d)} SRA(s) downloads:"
        )
        cerr(
            "\n".join(
                f"{sra_id}: {msg}" for (sra_id, msg) in fetcher.sra_errors.items()
            )
        )
...
class SampleFileIterator(object):
    ...
    def __init__(self, samplefile):
        ...
        if ":" in self.samplefile:
            self.samplefile, column_specs = self.samplefile.split(":")
            self.sample_column, self.ena_column = column_specs.split(",")
        else:
            self.sample_column, self.ena_column = "SAMPLE", "ENA"

        if self.samplefile.endswith(".xls") or self.samplefile.endswith(".xlsx"):
            self.df = pd.read_excel(self.samplefile)
        else:
            self.df = pd.read_table(self.samplefile, sep=None, engine="python")

    def __iter__(self):
        return self.iter_samplefile()

    def iter_samplefile(self):
        """return a list of (sample, [SRAID, ...])"""

        for idx, row in self.df.iterrows():

            sample = row[self.sample_column]
            ena_accs = row[self.ena_column]
