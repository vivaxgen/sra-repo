...
class SRAFileStorage(object):
    ...
    def __init__(self, storage_root_path: pathlib.Path):
        ...
        if not (self.__storage_root_path__ / ".sra-repo-db").is_file():
            cexit(
                f"ERROR: root fs {self.__storage_root_path__} is not a SRA repo storage"
            )
    ...
    def link(
        ...
        if flat:
            ...
            for a_file in target_link.iterdir():
                files.append(f"{a_file.parts[-2]}/{a_file.parts[-1]}")
    ...
    def get_read_files(self, sra_id: str):
        ...
        return [
            a_file for a_file in sra_dir.iterdir() if a_file.name.endswith(".fastq.gz")
        ]
                ...
                if verify:
                    for a_file in store_dir.iterdir():
                        ...
                        if not check_gzip_file(a_file):
                            raise ValueError(
                                f"SRA {sra_id} with file {a_file} is not verified!"
                            )
            except:
                if throw_exc:
                    raise
