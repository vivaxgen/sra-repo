...
def get_ena_filereport(
    ...
    while status_code != 200 and tries < 5:
        r = requests.get(
            "https://www.ebi.ac.uk/ena/portal/api/filereport", params=payload
        )
        ...
        if status_code != 200:
            raise ValueError(
                f"SRA accession not found: {sra_id} with HTTP Error {r.status_code} "
...
class ENA_Helper(object):
    ...
    def __init__(self, parent, showcmds=False, show_sra_info=False):
        self.parent = parent
    ...
    def process_file(self, path, sra):
        ...
        if path.name.endswith("fastq.gz"):
            ...
            try:
                fastq_filenames = [
                    path.with_suffix("_1.fastq.gz"),
                    path.with_suffix("_2.fastq.gz"),
                ]
...
def cram_to_fastq(localpath, destfiles):
    ...
    cmds = (
        f"samtools collate -u -f -O {localpath} | "
    ...
    if (
        subprocess.call(run_cmds, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        != 0
    ):
